import logo from "../images/team/t1.png"
import logo2 from "../images/team/t2.png"
import logo3 from "../images/team/t3.png"
import twitter from "../images/twitter.png"
import facebook from "../images/facebook.png"
import youtube from "../images/youtube_1384060.png"
import instagram from "../images/instagram_2111463.png"


export const TeamData = [
  {
    id: 1,
    img:logo,
    title: "Zoe Fisher",
    exprience: "12 YEAR EXPERIENCE",
    socialmedia: [
        { type: twitter, link: "https://twitter.com/username1" },
        { type: facebook, link: "https://facebook.com/username1" },
        { type: youtube, link: "https://youtube.com/username1" },
        { type: instagram, link: "https://instagram.com/username1" },
      ],
  },
  {
    id: 2,
    img: logo2,
    title: "Cecil Wood",
    exprience: "18 YEAR EXPERIENCE",
    socialmedia: [
      { type: twitter, link: "https://twitter.com/username2" },
      { type: facebook, link: "https://facebook.com/username2" },
      { type: youtube, link: "https://youtube.com/username2" },
      { type: instagram, link: "https://instagram.com/username2" },
    ],
  },
  {
    id: 3,
    img: logo3,
    title: "Shane Harvey",
    exprience: "6 YEAR EXPERIENCE",
    socialmedia: [
        { type: twitter, link: "https://twitter.com/username3" },
        { type: facebook, link: "https://facebook.com/username3" },
        { type: youtube, link: "https://youtube.com/username3" },
        { type: instagram, link: "https://instagram.com/username3" },
      ],
  },
];

export const Drinks = [
  {
    id: 1,
    title: "Cola",
    description: "Cola is a carbonated soft drink flavored with vanilla, cinnamon, citrus oils, and other flavorings. Cola became popular worldwide after the American pharmacist John Stith Pemberton invented Coca-Cola, a trademarked brand, in 1886, which was imitated by other manufacturers.",
    price: 4.99
  },
  {
    id: 2,
    title: "Lemonade",
    description: "Lemonade is a sweetened lemon-flavored beverage.",
    price: 3.49
  },
  {
    id: 3,
    title: "Orange Juice",
    description: "Orange juice is the liquid extract of the fruit of the orange tree.",
    price: 2.99
  },
  {
    id: 4,
    title: "Iced Tea",
    description: "Iced tea is a tea that has been cooled down and served over ice.",
    price: 1.99
  },
  {
    id: 5,
    title: "Apple Cider",
    description: "Apple cider is a beverage made from the fermentation of apple juice.",
    price: 5.49
  },
  {
    id: 6,
    title: "Ginger Ale",
    description: "Ginger ale is a carbonated soft drink flavored with ginger.",
    price: 3.99
  },
  {
    id: 7,
    title: "Cranberry Juice",
    description: "Cranberry juice is a tart red-colored juice made from cranberries.",
    price: 4.49
  },
  {
    id: 8,
    title: "Pineapple Juice",
    description: "Pineapple juice is a sweet and tangy juice made from pineapples.",
    price: 3.79
  },
  {
    id: 9,
    title: "Grapefruit Juice",
    description: "Grapefruit juice is a tart and tangy juice made from grapefruits.",
    price: 3.99
  },
  {
    id: 10,
    title: "Peach Iced Tea",
    description: "Peach iced tea is a sweetened iced tea flavored with peach.",
    price: 4.29
  },
  {
    id: 11,
    title: "Limeade",
    description: "Limeade is a sweetened lime-flavored beverage.",
    price: 3.49
  },
  {
    id: 12,
    title: "Mango Juice",
    description: "Mango juice is a sweet and tropical juice made from mangoes.",
    price: 4.99
  },
  {
    id: 13,
    title: "Watermelon Juice",
    description: "Watermelon juice is a refreshing juice made from watermelons.",
    price: 3.99
  },
  {
    id: 14,
    title: "Iced Coffee",
    description: "Iced coffee is a chilled coffee beverage.",
    price: 4.49
  },
  {
    id: 15,
    title: "Pomegranate Juice",
    description: "Pomegranate juice is a tart and tangy juice made from pomegranates.",
    price: 5.29
  },
  {
    id: 16,
    title: "Pear Juice",
    description: "Pear juice is a sweet and mild juice made from pears.",
    price: 3.49
  },
  {
    id: 17,
    title: "Raspberry Iced Tea",
    description: "Raspberry iced tea is a sweetened iced tea flavored with raspberry.",
    price: 4.29
  },
  {
    id: 18,
    title: "Blueberry Juice",
    description: "Blueberry juice is a sweet and tangy juice made from blueberries.",
    price: 4.99
  },
  {
    id: 19,
    title: "Mint Lemonade",
    description: "Mint lemonade is a sweetened lemon-flavored beverage infused with mint.",
    price: 3.99
  },
  {
    id: 20,
    title: "Coconut Water",
    description: "Coconut water is a clear liquid extracted from green coconuts.",
    price: 2.49
  }
];
export const Foods = [
  {
    id: 1,
    title: "Classic Burger"
  },
  {
    id: 2,
    title: "Cheeseburger"
  },
  {
    id: 3,
    title: "Bacon Burger"
  },
  {
    id: 4,
    title: "Mushroom Swiss Burger"
  },
  {
    id: 5,
    title: "BBQ Bacon Burger"
  },
  {
    id: 6,
    title: "Avocado Burger"
  },
  {
    id: 7,
    title: "Turkey Burger"
  },
  {
    id: 8,
    title: "Veggie Burger"
  },
  {
    id: 9,
    title: "Teriyaki Burger"
  },
  {
    id: 10,
    title: "Western Burger"
  },
  {
    id: 11,
    title: "Chili Burger"
  },
  {
    id: 12,
    title: "Guacamole Burger"
  },
  {
    id: 13,
    title: "Bacon Ranch Burger"
  },
  {
    id: 14,
    title: "Patty Melt"
  },
  {
    id: 15,
    title: "Jalapeno Burger"
  },
  {
    id: 16,
    title: "Hawaiian Burger"
  },
  {
    id: 17,
    title: "Black Bean Burger"
  },
  {
    id: 18,
    title: "Buffalo Burger"
  },
  {
    id: 19,
    title: "Crispy Chicken Burger"
  },
  {
    id: 20,
    title: "Grilled Chicken Burger"
  }
];
export const Sweets = [
  {
    id: 1,
    title: "Cheesecake"
  },
  {
    id: 2,
    title: "Chocolate Cake"
  },
  {
    id: 3,
    title: "Apple Pie"
  },
  {
    id: 4,
    title: "Brownie"
  },
  {
    id: 5,
    title: "Ice Cream Sundae"
  },
  {
    id: 6,
    title: "Tiramisu"
  },
  {
    id: 7,
    title: "Cupcake"
  },
  {
    id: 8,
    title: "Lemon Tart"
  },
  {
    id: 9,
    title: "Red Velvet Cake"
  },
  {
    id: 10,
    title: "Pecan Pie"
  },
  {
    id: 11,
    title: "Key Lime Pie"
  },
  {
    id: 12,
    title: "Chocolate Chip Cookie"
  },
  {
    id: 13,
    title: "Strawberry Shortcake"
  },
  {
    id: 14,
    title: "Raspberry Tart"
  },
  {
    id: 15,
    title: "Blueberry Muffin"
  },
  {
    id: 16,
    title: "Banana Bread"
  },
  {
    id: 17,
    title: "Peach Cobbler"
  },
  {
    id: 18,
    title: "Coconut Macaroons"
  },
  {
    id: 19,
    title: "Pumpkin Pie"
  },
  {
    id: 20,
    title: "Cinnamon Roll"
  }
];
export const Pizza = [
  {
    id: 1,
    title: "Margarita Pizza"
  },
  {
    id: 2,
    title: "Pepperoni Pizza"
  },
  {
    id: 3,
    title: "Vegetarian Pizza"
  },
  {
    id: 4,
    title: "Hawaiian Pizza"
  },
  {
    id: 5,
    title: "BBQ Chicken Pizza"
  },
  {
    id: 6,
    title: "Supreme Pizza"
  },
  {
    id: 7,
    title: "Meat Lovers Pizza"
  },
  {
    id: 8,
    title: "Buffalo Chicken Pizza"
  },
  {
    id: 9,
    title: "Four Cheese Pizza"
  },
  {
    id: 10,
    title: "White Pizza"
  },
  {
    id: 11,
    title: "Sausage Pizza"
  },
  {
    id: 12,
    title: "Mushroom Pizza"
  },
  {
    id: 13,
    title: "Spinach Pizza"
  },
  {
    id: 14,
    title: "Artichoke Pizza"
  },
  {
    id: 15,
    title: "Barbecue Bacon Pizza"
  },
  {
    id: 16,
    title: "Pesto Pizza"
  },
  {
    id: 17,
    title: "Taco Pizza"
  },
  {
    id: 18,
    title: "Chicken Alfredo Pizza"
  },
  {
    id: 19,
    title: "Buffalo Cauliflower Pizza"
  },
  {
    id: 20,
    title: "Greek Pizza"
  }
];
